...
<body>
	<?php
		...
		if (isset($_GET["codprod"])){
			...
			$prod=new Producto();
			$p=$prod->BuscarProductoPorCodigo($codprod); 
				...
				<div class="row justify-content-center">
					<div class="col-md-4">
						<form name="frmregistro" id="frmregistro" method="post" action="../controler/grabar.php">
							<input type="hidden" name="txttipo" id="txttipo" value="e" />
							<div class="mb-3">
    							<label for="txtcodigo" class="form-label">Codigo de Producto</label>
    							<input type="text" class="form-control" name="txtcodigo" id="txtcodigo" readonly value="<?=$p->codigo_producto?>" />
  							</div>
  							<div class="mb-3">
    							<label for="txtproducto" class="form-label">Producto</label>
    							<input type="text" class="form-control" name="txtproducto" id="txtproducto" value="<?=$p->producto?>" />
  							</div>
  							<div class="mb-3">
    							<label for="txtstok" class="form-label">Stok</label>
    							<input type="number" class="form-control" name="txtstok" id="txtstok" min="1" max="1000" value="<?=$p->stok_disponible?>" />
  							</div>
  							<div class="mb-3">
    							<label for="txtcosto" class="form-label">Costo</label>
    							<input type="text" class="form-control" name="txtcosto" id="txtcosto" value="<?=$p->costo?>" />
  							</div>
  							<div class="mb-3">
    							<label for="txtganancia" class="form-label">Ganancia</label>
    							<input type="text" class="form-control" name="txtganancia" id="txtganancia" value="<?=$p->ganancia?>" />
  							</div>
  							<div class="mb-3">
